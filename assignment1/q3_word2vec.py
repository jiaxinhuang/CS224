...
def normalizeRows(x):
    ...
    legend = np.sqrt(np.sum(np.square(x), axis=1))
    x /= legend[np.newaxis].T
    ### END YOUR CODE
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    M, N = outputVectors.shape
    delta = yhat
    delta[target] -= 1
    gradPred = np.dot(delta, outputVectors)
    grad = np.dot(delta.reshape((M, 1)), predicted.reshape((1, N)))
    ### END YOUR CODE
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    # print '-'*10
    # print outputVectors.size, predicted.shape
    cost = -np.log(z)
    ...
    # for k in range(K):
    #     sign = indices[k+1]
    #     z_k = sigmoid(-np.dot(outputVectors[sign], predicted))
    #     cost -= np.log(z_k)
    #     gradPred -= (z_k - 1.0) * outputVectors[sign]
    #     grad[sign] = (1.0 - z_k) * outputVectors[target]
    ### END YOUR CODE
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    # print '-'*10
    # print inputVectors.shape, outputVectors.shape
