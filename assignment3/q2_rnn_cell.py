...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
	    ...
	    W_x = tf.get_variable(shape=[self.input_size, self._state_size], initializer=tf.contrib.layers.xavier_initializer(), name="W_x", dtype=tf.float32)
	    W_h = tf.get_variable(shape=[self._state_size, self._state_size], initializer=tf.contrib.layers.xavier_initializer(), name="W_h", dtype=tf.float32)
	    b = tf.get_variable(shape=[self._state_size,], initializer=tf.contrib.layers.xavier_initializer(), name="b", dtype=tf.float32)
