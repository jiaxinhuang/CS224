...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
	    ...
	    initFun = tf.contrib.layers.xavier_initializer()
	    W_r = tf.get_variable(shape=[self.state_size, self.state_size], initializer=initFun, name="W_r")
	    W_z = tf.get_variable(shape=[self.state_size, self.state_size], initializer=initFun, name="W_z")
	    W_o = tf.get_variable(shape=[self.state_size, self.state_size], initializer=initFun, name="W_o")

	    U_r = tf.get_variable(shape=[self.input_size, self.state_size], initializer=initFun, name="U_r")
	    U_z = tf.get_variable(shape=[self.input_size, self.state_size], initializer=initFun, name="U_z")
	    U_o = tf.get_variable(shape=[self.input_size, self.state_size], initializer=initFun, name="U_o")

	    b_r = tf.get_variable(shape=[self.state_size,], initializer=tf.constant_initializer(0.0), name="b_r")
	    b_z = tf.get_variable(shape=[self.state_size,], initializer=tf.constant_initializer(0.0), name="b_z")
	    b_o = tf.get_variable(shape=[self.state_size,], initializer=tf.constant_initializer(0.0), name="b_o")
